# This Makefile is for the LendingClub::API extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/LendingClub/API.pm]
...
#     NAME => q[LendingClub::API]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0], LWP::UserAgent=>q[0], Hash::Flatten=>q[0], JSON=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/LendingClub/API.pm]
#     clean => { FILES=>q[LendingClub-API-*] }
...
MAKE = make
FULLEXT = LendingClub/API
BASEEXT = API
PARENT_NAME = LendingClub
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/LendingClub/API.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/LendingClub
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/LendingClub
...
TO_INST_PM = lib/LendingClub/API.pm

PM_TO_BLIB = lib/LendingClub/API.pm \
...
DISTNAME = LendingClub-API
DISTVNAME = LendingClub-API-0.3.0
...
manifypods : pure_all  \
	lib/LendingClub/API.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/LendingClub/API.pm $(INST_MAN3DIR)/LendingClub::API.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) '  - '\''Michael Renz <cryptographrix+cpan@gmail.com>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: LendingClub-API' >> META_new.yml
	$(NOECHO) $(ECHO) 'no_index:' >> META_new.yml
	$(NOECHO) $(ECHO) '  directory:' >> META_new.yml
	$(NOECHO) $(ECHO) '    - t' >> META_new.yml
	$(NOECHO) $(ECHO) '    - inc' >> META_new.yml
	$(NOECHO) $(ECHO) 'requires:' >> META_new.yml
	$(NOECHO) $(ECHO) '  Hash::Flatten: 0' >> META_new.yml
	$(NOECHO) $(ECHO) '  JSON: 0' >> META_new.yml
	$(NOECHO) $(ECHO) '  LWP::UserAgent: 0' >> META_new.yml
	$(NOECHO) $(ECHO) '  perl: 5.006' >> META_new.yml
	$(NOECHO) $(ECHO) 'version: v0.3.0' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "unknown",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Michael Renz <cryptographrix+cpan@gmail.com>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "LendingClub-API",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '         "requires" : {' >> META_new.json
	$(NOECHO) $(ECHO) '            "Hash::Flatten" : "0",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   "version" : "v0.3.0"' >> META_new.json
	$(NOECHO) $(ECHO) '}' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <AUTHOR>Michael Renz &lt;cryptographrix+cpan@gmail.com&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,006,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Hash::Flatten" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/LendingClub/API.pm blib/lib/LendingClub/API.pm 
